  ...
  return (
    <div onMouseDown={() => audioService.init()} className="w-full h-screen bg-black p-2 md:p-6 overflow-hidden">
      <div className="w-full h-full crt-frame flex flex-col md:flex-row relative">
        ...
        {/* --- DIESER TEIL ZENTRIERT DAS SPIEL --- */}
        <main className="flex-1 relative bg-black flex items-center justify-center z-10 overflow-auto">
          {currentView === 'menu' && (
            <div className="font-vt323 text-neon-cyan text-center p-10">
              <h2 className="text-8xl opacity-10 mb-4 select-none">NEXUS</h2>
              <div className="p-8 border border-neon-cyan/20 bg-neon-cyan/5 max-w-lg">
                <p className="text-xl mb-6 italic tracking-widest uppercase">Kernel Ready</p>
                <button onClick={() => setCurrentView('snake')} className="w-full px-10 py-3 border-2 border-neon-cyan hover:bg-neon-cyan hover:text-black font-orbitron text-xs transition-all shadow-[0_0_15px_rgba(0,243,255,0.2)]">BOOT_SNAKE.EXE</button>
              </div>
            </div>
          )}
