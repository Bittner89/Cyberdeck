          ...
          {loading ? (
            ...
            scores.map((entry, i) => (
              <div 
                ...
                className={`flex items-center p-3 rounded-xl border ${entry.username?.toUpperCase() === user?.toUpperCase() ? 'bg-neon-cyan/10 border-neon-cyan' : 'bg-[#050a0a] border-neon-cyan/10'}`}
              >
                <span className={`text-xl w-8 font-bold ${i < 3 ? 'text-neon-pink' : 'text-neon-cyan/40'}`}>#{i + 1}</span>
                <span className="text-lg flex-1 truncate">{entry.username?.toUpperCase() || 'ANONYMOUS'}</span>
                <span className="text-xl text-white tracking-widest">{entry.score.toLocaleString()}</span>
              </div>
            ))
          ) : (
  ...
  return (
    <div className="w-full lg:w-[1164px] lg:max-w-full max-w-4xl lg:h-[850px] max-h-full flex flex-col animate-glitch-entry font-vt323 p-3 md:p-8 bg-black/90 border-2 border-neon-cyan shadow-neon-big relative z-20 chamfer">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-3 md:gap-0 mb-4 md:mb-8 border-b-2 border-neon-cyan pb-4">
        <h2 className="text-2xl md:text-4xl text-neon-cyan tracking-tighter uppercase italic glitch-hover text-center md:text-left">
          {activeTab === 'global' ? '::Global_Leaderboard::' : '::Personal_Archive::'}
        </h2>
        <button 
          onClick={() => navigate('menu')} 
          className="text-neon-pink hover:bg-neon-pink hover:text-black border border-neon-pink w-full md:w-auto px-4 md:px-6 py-1 transition-all chamfer-btn glitch-hover text-sm md:text-base"
        >
          BACK_TO_MENU
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:flex md:flex-wrap gap-2 md:gap-4 mb-4">
        <button 
          ...
          className={`flex-1 py-1 border transition-all duration-300 chamfer-btn glitch-hover text-xs md:text-base ${activeGame === 'snake' ? 'bg-neon-cyan/20 text-white border-neon-cyan shadow-[0_0_10px_rgba(0,243,255,0.3)]' : 'text-neon-cyan/50 border-neon-cyan/20 hover:border-neon-cyan/50 hover:text-neon-cyan bg-black/50'}`}
        >
          GAME: NEURAL_SNAKE
        </button>
        <button 
          onClick={() => setActiveGame('tetris')}
          className={`flex-1 py-1 border transition-all duration-300 chamfer-btn glitch-hover text-xs md:text-base ${activeGame === 'tetris' ? 'bg-neon-cyan/20 text-white border-neon-cyan shadow-[0_0_10px_rgba(0,243,255,0.3)]' : 'text-neon-cyan/50 border-neon-cyan/20 hover:border-neon-cyan/50 hover:text-neon-cyan bg-black/50'}`}
        >
          GAME: BLOCK_ENCRYPT
        </button>
      <button 
        onClick={() => setActiveGame('spaceinvaders')}
        className={`flex-1 py-1 border transition-all duration-300 chamfer-btn glitch-hover text-xs md:text-base ${activeGame === 'spaceinvaders' ? 'bg-neon-cyan/20 text-white border-neon-cyan shadow-[0_0_10px_rgba(0,243,255,0.3)]' : 'text-neon-cyan/50 border-neon-cyan/20 hover:border-neon-cyan/50 hover:text-neon-cyan bg-black/50'}`}
      >
        GAME: ALIEN_THREAT
      </button>
        <button 
          onClick={() => setActiveGame('breakout')}
          className={`flex-1 py-1 border transition-all duration-300 chamfer-btn glitch-hover text-xs md:text-base ${activeGame === 'breakout' ? 'bg-neon-cyan/20 text-white border-neon-cyan shadow-[0_0_10px_rgba(0,243,255,0.3)]' : 'text-neon-cyan/50 border-neon-cyan/20 hover:border-neon-cyan/50 hover:text-neon-cyan bg-black/50'}`}
        >
      ...
      <div className="flex gap-2 md:gap-4 mb-4 md:mb-6">
        <button 
          ...
          className={`flex-1 py-2 border-2 transition-all duration-300 chamfer-btn glitch-hover text-xs md:text-base ${activeTab === 'global' ? 'bg-neon-cyan text-black border-neon-cyan shadow-neon' : 'bg-black/50 text-neon-cyan border-neon-cyan/30 hover:border-neon-cyan'}`}
        >
          GLOBAL_NET_SCORES
        </button>
        <button 
          onClick={() => setActiveTab('personal')}
          className={`flex-1 py-2 border-2 transition-all duration-300 chamfer-btn glitch-hover text-xs md:text-base ${activeTab === 'personal' ? 'bg-neon-pink text-black border-neon-pink shadow-neon-pink' : 'bg-black/50 text-neon-pink border-neon-pink/30 hover:border-neon-pink'}`}
        >
        ...
        {loading ? (
          ...
          scores.map((entry, i) => (
            <div 
              ...
              className={`flex items-center justify-between p-2 md:p-3 border transition-colors group ${
            ...
            >
              <div className="flex items-center gap-2 md:gap-6">
                <span className={`text-lg md:text-2xl w-6 md:w-8 shrink-0 ${i < 3 ? 'text-neon-pink' : 'opacity-30'}`}>#{i + 1}</span>
                <span className="text-base md:text-xl tracking-widest group-hover:text-neon-cyan transition-colors truncate max-w-[100px] sm:max-w-[200px] md:max-w-none">
              ...
              <div className="flex items-center gap-2 md:gap-12">
                ...
                <span className="text-lg md:text-2xl text-white shadow-neon w-16 md:w-24 text-right shrink-0">
